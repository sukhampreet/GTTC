...
import type { HumidityReading } from '@/modules/environment-monitoring/types';

export interface HumidityTableProps {
  sensors: HumidityReading[];
}

const columns: DataTableColumn<HumidityReading>[] = [
  ...
  { id: 'current', header: 'Current Reading', cell: (r) => <span className="tabular-nums">{r.current}%</span>, sortAccessor: (r) => r.current, align: 'right' },
  { id: 'min', header: 'Min', cell: (r) => <span className="tabular-nums">{r.min}%</span>, sortAccessor: (r) => r.min, align: 'right' },
  { id: 'max', header: 'Max', cell: (r) => <span className="tabular-nums">{r.max}%</span>, sortAccessor: (r) => r.max, align: 'right' },
  {
    id: 'status',
    header: 'Status',
...
export function HumidityTable({ sensors }: HumidityTableProps) {
  return (
    <DataTable
      data={sensors}
      columns={columns}
      ...
      pageSize={8}
      emptyTitle="No humidity sensors found"
