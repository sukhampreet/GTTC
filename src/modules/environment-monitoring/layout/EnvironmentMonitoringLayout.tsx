...
import { ENVIRONMENT_MONITORING_SUBNAV } from '@/modules/environment-monitoring/constants/subnav';

/**
 * Module-local secondary navigation. Sprint 1's global Sidebar only renders
 * top-level modules, so Environment Monitoring's pages are exposed here as an
 ...
 */
export function EnvironmentMonitoringLayout() {
  return (
    <div className="flex flex-col gap-4">
      <nav
        aria-label="Environment Monitoring sections"
        className="flex flex-wrap gap-1 overflow-x-auto rounded-(--radius-lg) border border-border-default bg-surface p-1.5"
      >
        {ENVIRONMENT_MONITORING_SUBNAV.map((item) => {
          const Icon = item.icon;
          const to = item.path ? `${ROUTES.environmentMonitoring}/${item.path}` : ROUTES.environmentMonitoring;
          return (
            <NavLink
              key={item.id}
              to={to}
