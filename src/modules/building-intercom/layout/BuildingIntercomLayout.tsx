...
import { BUILDING_INTERCOM_SUBNAV } from '@/modules/building-intercom/constants/subnav';

/**
 * Module-local secondary navigation. Sprint 1's global Sidebar only renders
 * top-level modules, so Building Intercom's pages are exposed here as an
 ...
 */
export function BuildingIntercomLayout() {
  return (
    <div className="flex flex-col gap-4">
      <nav
        aria-label="Building Intercom sections"
        className="flex flex-wrap gap-1 overflow-x-auto rounded-(--radius-lg) border border-border-default bg-surface p-1.5"
      >
        {BUILDING_INTERCOM_SUBNAV.map((item) => {
          const Icon = item.icon;
          const to = item.path ? `${ROUTES.buildingIntercom}/${item.path}` : ROUTES.buildingIntercom;
          return (
            <NavLink
              key={item.id}
              to={to}
