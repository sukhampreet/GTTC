...
import { FIRE_EMERGENCY_SUBNAV } from '@/modules/fire-emergency/constants/subnav';

/**
 * Module-local secondary navigation. Sprint 1's global Sidebar only renders
 * top-level modules, so Fire & Emergency's pages are exposed here as an
 ...
 */
export function FireEmergencyLayout() {
  return (
    <div className="flex flex-col gap-4">
      <nav
        aria-label="Fire & Emergency sections"
        className="flex flex-wrap gap-1 overflow-x-auto rounded-(--radius-lg) border border-border-default bg-surface p-1.5"
      >
        {FIRE_EMERGENCY_SUBNAV.map((item) => {
          const Icon = item.icon;
          const to = item.path ? `${ROUTES.fireEmergency}/${item.path}` : ROUTES.fireEmergency;
          return (
            <NavLink
              key={item.id}
              to={to}
