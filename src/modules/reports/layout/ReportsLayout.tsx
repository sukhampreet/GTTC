...
import { REPORTS_SUBNAV } from '@/modules/reports/constants/subnav';
 ...
 * by every other subsystem module (Fire & Emergency, AI Analytics, etc).
...
export function ReportsLayout() {
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        {REPORTS_SUBNAV.map((item) => {
          const Icon = item.icon;
          const to = item.path ? `${ROUTES.reports}/${item.path}` : ROUTES.reports;
          return (
            <NavLink
              key={item.id}
              to={to}
