...
export interface ExitActivityTableProps {
  events: ParkingEvent[];
}

export function ExitActivityTable({ events }: ExitActivityTableProps) {
  return (
    <AppCard className="overflow-hidden">
      <AppCardHeader>
        <AppCardTitle>Recent Exit Activity</AppCardTitle>
      </AppCardHeader>
      {events.length === 0 ? (
        <EmptyState title="No recent exits" description="No vehicles have exited through this gate recently." />
      ) : (
          ...
          <table className="w-full border-collapse text-left text-[12.5px]">
            <thead>
              <tr className="border-b border-border-default bg-surface-raised/60">
                ...
                <th className="whitespace-nowrap px-3.5 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-text-tertiary">Slot Vacated</th>
              </tr>
            </thead>
            <tbody>
              {events.map((event) => (
