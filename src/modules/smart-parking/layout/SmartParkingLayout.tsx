...
import { SMART_PARKING_SUBNAV } from '@/modules/smart-parking/constants/subnav';

/**
 * Module-local secondary navigation. Sprint 1's global Sidebar only renders
 * top-level modules, so Smart Parking's pages are exposed here as an
 ...
 */
export function SmartParkingLayout() {
  return (
    <div className="flex flex-col gap-4">
      <nav
        aria-label="Smart Parking sections"
        className="flex flex-wrap gap-1 overflow-x-auto rounded-(--radius-lg) border border-border-default bg-surface p-1.5"
      >
        {SMART_PARKING_SUBNAV.map((item) => {
          const Icon = item.icon;
          const to = item.path ? `${ROUTES.smartParking}/${item.path}` : ROUTES.smartParking;
          return (
            <NavLink
              key={item.id}
              to={to}
