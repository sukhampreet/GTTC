...
const columns: DataTableColumn<PPEDetectionEvent>[] = [
  ...
  { id: 'violation', header: 'Violation', cell: (r) => titleCase(r.violation), sortAccessor: (r) => r.violation },
...
export function PPEDetectionTable({ events }: { events: PPEDetectionEvent[] }) {
  return (
    <DataTable
      data={events}
      columns={columns}
      getRowId={(r) => r.id}
      searchableText={(r) => `${r.camera} ${r.location} ${r.violation}`}
      searchPlaceholder="Search by camera, location or violation…"
      pageSize={8}
      emptyTitle="No PPE violations found"
