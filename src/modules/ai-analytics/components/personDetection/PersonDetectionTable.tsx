...
const columns: DataTableColumn<PersonDetectionEvent>[] = [
  ...
  { id: 'count', header: 'Count', cell: (r) => <span className="tabular-nums">{r.count}</span>, sortAccessor: (r) => r.count, align: 'right' },
...
export function PersonDetectionTable({ events }: { events: PersonDetectionEvent[] }) {
  return (
    <DataTable
      data={events}
      columns={columns}
      getRowId={(r) => r.id}
      searchableText={(r) => `${r.camera} ${r.location}`}
      searchPlaceholder="Search by camera or location…"
      pageSize={8}
      emptyTitle="No person detections found"
